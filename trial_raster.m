...
%   swtteo; wavelet energy operator
    ...
    switch sd
        case 'thresh'
            if ~exist(fullfile(C.Dir{idx},[char(C.Blocks(idx))],[char(C.Blocks(idx)),'_TC-neg3.5_ThreshCross']))
                disp('Run spike detection')
            end
            load(fullfile(C.Dir{idx},[char(C.Blocks(idx))],[char(C.Blocks(idx)),'_StimTimes.mat']));
            D = dir(fullfile(C.Dir{idx},[char(C.Blocks(idx))],[char(C.Blocks(idx)),'_TC-neg3.5_ThreshCross'], ...
            ...
            T = cell(size(D,1),1);
            for i = 1:numel(D)
                load(fullfile(C.Dir{idx},[char(C.Blocks(idx))],[char(C.Blocks(idx)),'_TC-neg3.5_ThreshCross'], ...
            ...
            if exist(fullfile(C.Dir{idx},[char(C.Blocks(idx))],[char(C.Blocks(idx)),'_refstats.mat']),'file')
                load(fullfile(C.Dir{idx},[char(C.Blocks(idx))],[char(C.Blocks(idx)),'_refstats']))
                chPlot.trials = T;
                save(fullfile(C.Dir{idx},[char(C.Blocks(idx))],[char(C.Blocks(idx)),'_refstats']),'chPlot');
            else
                disp('run channel_stats.m first')
                return
            end
        case 'swtteo'
            if ~exist(fullfile(C.Dir{idx},[char(C.Blocks(idx))],[char(C.Blocks(idx)),'_SD_SWTTEO']))
                disp('Run spike detection')
            end
            load(fullfile(C.Dir{idx},[char(C.Blocks(idx))],[char(C.Blocks(idx)),'_StimTimes.mat']));
            D = dir(fullfile(C.Dir{idx},[char(C.Blocks(idx))],[char(C.Blocks(idx)),'_SD_SWTTEO'], ...
                [char(C.Blocks(idx)),'_ptrain' '*Ch*.mat']));
            name = string({D.name})';
            meta = split(name,"_");
            meta(:,9:10) = split(meta(:,9),'.');
            ...
            T = cell(size(D,1),1);
            for i = 1:numel(D)
                load(fullfile(C.Dir{idx},[char(C.Blocks(idx))],[char(C.Blocks(idx)),'_SD_SWTTEO']), ...
                    [char(C.Blocks(idx)) '_ptrain_' meta(i,1) '_Ch_' meta(i,2)]);
                sp = [];
                for ii = 1:numel(StimOffsets)
                    trial = StimOffsets(ii);
                    fill = peak_train(trial:trial + 300);
